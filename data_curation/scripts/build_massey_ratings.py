...
def build_game_scores() -> pd.DataFrame:
    ...
    gs = load_game_summaries(DATA_DIR)
    ...
    quarter_path = DATA_DIR / "TeamQuarterScores.parquet"
...
def _fit_all_ratings_for_date(season_games, game_date, season):
    """
...
def compute_pregame_ratings(games: pd.DataFrame, min_season: str | None = None) -> pd.DataFrame:
    ...
    for i, season in enumerate(seasons):
        ...
        results = Parallel(n_jobs=n_jobs, backend="loky")(
            delayed(_fit_all_ratings_for_date)(season_games, gd, season)
