...
# --- CONFIGURATION ---
DATA_DIR = Path('/Users/michaelharoon/Projects/prediction_markets/nba/data_curation/data/raw_payloads/hustle')
LOG_PATH = Path('/Users/michaelharoon/Projects/prediction_markets/nba/data_curation/logs/hustle.log')
...
def log_message(game_id, status, message):
    ...
    logging.info(f"[{timestamp}] [Game: {game_id}] [{status}] {message}")


def fetch_game_unrelenting(args):
    game_id, index = args
    output_path = DATA_DIR / f"{game_id}_hustle.json"

    # Reset session every N games by sleeping briefly to close any lingering TCP state
    ...
    while not success:
        try:
            ...
            log_message(game_id, "FETCHING", f"Requesting payload (Attempt {retries + 1})...")

            summary = BoxScoreHustleV2(
                game_id=game_id,
                headers=HEADERS,
                timeout=18
            )
            ...
            retries += 1
            base_wait = random.uniform(40.0, 70.0)
...
if __name__ == "__main__":
    ...
    existing_files = {f.name.split('_')[0] for f in DATA_DIR.glob('*_hustle.json')}
    game_ids = [gid for gid in game_ids if gid not in existing_files]

    # Process newest games first so current-season data is captured even if blocked mid-run
    game_ids = game_ids[::-1]

    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Launching engine. Queue: {len(game_ids)} games. Log: {LOG_PATH}")

    indexed = list(enumerate(game_ids))
    indexed_swapped = [(gid, i) for i, gid in indexed]

    with ThreadPoolExecutor(max_workers=WORKERS) as executor:
        executor.map(fetch_game_unrelenting, indexed_swapped)
