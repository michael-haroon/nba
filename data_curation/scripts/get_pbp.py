...
# --- CONFIGURATION ---
DATA_DIR = Path('/Users/michaelharoon/Projects/prediction_markets/nba/data_curation/data/raw_payloads/pbp')
LOG_PATH = Path('/Users/michaelharoon/Projects/prediction_markets/nba/data_curation/logs/pbp.log')
...
def fetch_game_pbp_unrelenting(args):
    game_id, index = args
    output_path = DATA_DIR / f"{game_id}_pbp.json"

    # Reset session every N games
    ...
    while not success:
        try:
            ...
            log_message(game_id, "FETCHING", f"Requesting PBP payload (Attempt {retries + 1})...")
            ...
            raw_payload = pbp.nba_response.get_json()

            with open(output_path, 'w', encoding='utf-8') as f:
                json.dump(raw_payload, f, ensure_ascii=False)

            log_message(game_id, "SUCCESS", f"Saved raw PBP payload to {output_path.name}")
            success = True
            return True
...
if __name__ == "__main__":
    ...
    existing_files = {f.name.split('_')[0] for f in DATA_DIR.glob('*_pbp.json')}
    game_ids = [gid for gid in game_ids if gid not in existing_files]

    # Process newest games first
    game_ids = game_ids[::-1]

    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Launching PBP engine. Queue: {len(game_ids)} games. Log: {LOG_PATH}")

    indexed_swapped = [(gid, i) for i, gid in enumerate(game_ids)]

    with ThreadPoolExecutor(max_workers=WORKERS) as executor:
        executor.map(fetch_game_pbp_unrelenting, indexed_swapped)

    print("All PBP tasks processed.")
